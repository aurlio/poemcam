...
export function LoadingModal({ isOpen, language }: LoadingModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black bg-opacity-50 flex items-center justify-center" data-testid="loading-modal">
      <div className="bg-white dark:bg-gray-800 rounded-2xl p-8 mx-4 max-w-sm w-full text-center">
        ...
        <h3 className={`text-lg font-semibold text-gray-900 dark:text-white mb-2 ${language === 'zh' ? 'chinese-text' : ''}`} data-testid="text-loading-title">
          {t(language, 'creatingPoem')}
        </h3>
        <p className={`text-gray-600 dark:text-gray-400 ${language === 'zh' ? 'chinese-text' : ''}`} data-testid="text-loading-description">
